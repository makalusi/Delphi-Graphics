...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (testimonialCarousel) {
        const updateProgress = () => {
            const scrollPercent = (testimonialCarousel.scrollLeft / (testimonialCarousel.scrollWidth - testimonialCarousel.clientWidth)) * 100;
            if (testimonialProgress) {
                testimonialProgress.style.width = `${Math.max(10, scrollPercent)}%`;
            }
        };

        testimonialCarousel.addEventListener('scroll', updateProgress);

        if (testimonialNext) {
            testimonialNext.addEventListener('click', () => {
                const cardWidth = testimonialCarousel.firstElementChild.clientWidth + 32; // card + gap
        ...
        if (testimonialPrev) {
            testimonialPrev.addEventListener('click', () => {
                const cardWidth = testimonialCarousel.firstElementChild.clientWidth + 32;
                testimonialCarousel.scrollBy({ left: -cardWidth, behavior: 'smooth' });
            });
        }
